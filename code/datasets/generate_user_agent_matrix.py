...
def generate_user_agent_api_matrix(log_dir=None, output_csv=None):
    ...
    if output_csv is None:
        ...
        output_csv = os.path.join(output_dir, "user_agent_api_matrix.csv")

    # Auto-detect log directory candidates
    candidates = [
        log_dir,
        os.path.join(project_root, "flaws_cloudtrail_logs"),
        os.path.join(project_root, "datasets", "flaws_cloudtrail_logs"),
        os.path.join(os.path.dirname(project_root), "flaws_cloudtrail_logs"),
    ...
    print(
        f"Found {len(json_files)} JSON log files. Extracting userAgent and API call records..."
    )
    ...
    for idx, filepath in enumerate(json_files):
            ...
            with open(filepath, "r", encoding="utf-8") as f:
                ...
                for r in file_records:
                    if isinstance(r, dict):
                        ...
                        evt_name = (
                            r.get("eventName", "Unknown_Event") or "Unknown_Event"
                        )
                        evt_src = (
                            r.get("eventSource", "Unknown_Source") or "Unknown_Source"
                        )
        ...
        print(
            f"[{idx + 1}/{len(json_files)}] Read {os.path.basename(filepath)} in {time.time() - t0:.2f}s"
        )
    ...
    print(
        f"Matrix Shape: {matrix_df.shape[0]} UserAgents x {matrix_df.shape[1]} API Calls"
    )
