...
possible_csv_paths = [
    os.path.join(project_root, "output", "ip_api_matrix.csv"),
    os.path.join(project_root, "code", "ip_api_matrix.csv"),
    os.path.join(project_root, "ip_api_matrix.csv"),
    os.path.join(script_dir, "ip_api_matrix.csv"),
    os.path.join(project_root, "datasets", "ip_api_matrix.csv"),
    os.path.join(project_root, "data", "ip_api_matrix.csv"),
]
...
if csv_path is None:
    ...
    raise FileNotFoundError(
        f"ip_api_matrix.csv not found in any expected location.\n"
        f"Searched:\n  {searched}\n"
...
N_ip = len(ips)
N_api = len(api_names)
print(f"Loaded {N_ip} IP nodes and {N_api} API columns.")
...
ip_features = np.hstack([log_total_calls, out_degree, ent, max_call])
...
k = 20
print(f"Building sparse top-{k} nearest neighbor IP-IP graph...")
...
print(
    f"Constructed graph with {N_ip} nodes and {edge_indices.shape[1]} weighted edges."
)

graph_output_pt = os.path.join(output_dir, "ip_graph_data.pt")
torch.save(
    {
        "x": torch.tensor(ip_features, dtype=torch.float),
