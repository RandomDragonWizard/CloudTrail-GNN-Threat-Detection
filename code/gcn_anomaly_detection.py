...
possible_graph_paths = [
    os.path.join(project_root, "output", "ua_graph_data.pt"),
    os.path.join(project_root, "code", "actuall_code", "ua_graph_data.pt"),
    os.path.join(script_dir, "actuall_code", "ua_graph_data.pt"),
]
...
if graph_pt is None:
    ...
    raise FileNotFoundError(
        f"ua_graph_data.pt not found in any expected location.\n"
        f"Searched:\n  {searched}\n"
        f"Please run build_graph.py first."
    )
...
for epoch in range(1, epochs + 1):
    ...
    if epoch % 25 == 0 or epoch == 1:
        print(
            f"Epoch {epoch:03d} | Total Loss: {total_loss.item():.4f} | Feat Loss: {loss_feat.item():.4f} | Struct Loss: {loss_struct.item():.4f}"
        )
...
output_csv = os.path.join(output_dir, "ua_gcn_anomaly_scores.csv")
